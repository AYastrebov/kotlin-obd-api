...
class ResetTroubleCodesCommandTests {
    @Test
    fun `test reset trouble codes command properties`() {
        val command = ResetTroubleCodesCommand()
        assertEquals("RESET_TROUBLE_CODES", command.tag)
        assertEquals("Reset Trouble Codes", command.name)
        assertEquals("04", command.mode)
        assertEquals("", command.pid)
        assertEquals("04", command.rawCommand.trim())
    }
}
