    ...
    fun `test valid fuel air equivalence ratio responses handler`() {
        listOf(
            ...
            FuelAirEquivalenceRatioCommand.OxygenSensor.entries.forEach { sensor ->
                val obdResponse = FuelAirEquivalenceRatioCommand(sensor).run {
                    handleResponse(rawResponse)
                }
                assertEquals("${formatFloat(expected, 2)}F/A", obdResponse.formattedValue, "Failed for: $rawValue with sensor $sensor")
            }
        }
    }
}
