...
public class ModuleVoltageCommand : ObdCommand() {
    ...
    override val handler: (ObdRawResponse) -> String = { it: ObdRawResponse ->
        formatToDecimalPlaces(bytesToInt(it.bufferedValue) / 1000f, 2)
    }
}

public class TimingAdvanceCommand : ObdCommand() {
    ...
    override val handler: (ObdRawResponse) -> String = { it: ObdRawResponse ->
        formatToDecimalPlaces(bytesToInt(it.bufferedValue, bytesToProcess = 1) / 2f - 64, 2)
    }
}
...
public class VINCommand : ObdCommand() {
    ...
    override val defaultUnit: String = ""
    override val handler: (ObdRawResponse) -> String = { it: ObdRawResponse -> parseVIN(removeAll(it.value, WHITESPACE_PATTERN, BUS_INIT_PATTERN)) }
